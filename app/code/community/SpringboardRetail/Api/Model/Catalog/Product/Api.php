...
class SpringboardRetail_Api_Model_Catalog_Product_Api extends Mage_Catalog_Model_Product_Api
{
    public function create($type, $set, $sku, $productData, $store = null)
    {
        // Allow attribute set name instead of id
        if (is_string($set) && !is_numeric($set)) {
            $set = Mage::helper('springboardretail_api')->getAttributeSetIdByName($set);
        }
    ...
    protected function _prepareDataForSave($product, $productData)
    {
        ...
        if (isset($productData['categories'])) {
            $categoryIds = Mage::helper('springboardretail_api/catalog_product')
        ...
        foreach ($productData as $code => $value) {
            $productData[$code] = Mage::helper('springboardretail_api/catalog_product')
